...
fn clean_model_name_strips_latest_suffix() {
    assert_eq!(crate::model::utils::clean_model_name("model:latest"), "model");
}

#[test]
fn clean_model_name_preserves_alphanumeric_tag() {
    assert_eq!(crate::model::utils::clean_model_name("model:7b"), "model:7b");
}

#[test]
fn clean_model_name_strips_numeric_tag() {
    assert_eq!(crate::model::utils::clean_model_name("model:123"), "model");
}

#[test]
fn clean_model_name_no_colon_unchanged() {
    assert_eq!(crate::model::utils::clean_model_name("model"), "model");
}

#[test]
fn clean_model_name_empty_unchanged() {
    assert_eq!(crate::model::utils::clean_model_name(""), "");
}

#[test]
fn clean_model_name_strips_latest_then_evaluates_remaining() {
    assert_eq!(crate::model::utils::clean_model_name("model:7b:latest"), "model:7b");
}
...
fn extract_required_model_name_ok_when_present() {
    ...
    let got = crate::model::utils::extract_required_model_name(&body).expect("must accept");
...
fn extract_required_model_name_err_when_missing() {
    let body = json!({});
    let err = crate::model::utils::extract_required_model_name(&body).expect_err("must reject");
    assert_eq!(err.message, crate::constants::ERROR_MISSING_MODEL);
    assert_eq!(err.status_code, 400);
}

#[test]
fn extract_required_model_name_err_when_null() {
    let body = json!({ "model": serde_json::Value::Null });
    let err = crate::model::utils::extract_required_model_name(&body).expect_err("must reject");
    assert_eq!(err.message, crate::constants::ERROR_MISSING_MODEL);
}
...
fn extract_required_model_name_err_when_empty_string() {
    let body = json!({ "model": "" });
    let err =
        crate::model::utils::extract_required_model_name(&body).expect_err("must reject empty string");
    assert_eq!(err.message, crate::constants::ERROR_MISSING_MODEL);
}
...
fn extract_required_model_name_err_when_non_string() {
    ...
    let err =
        crate::model::utils::extract_required_model_name(&body).expect_err("must reject non-string");
    assert_eq!(err.message, crate::constants::ERROR_MISSING_MODEL);
}
