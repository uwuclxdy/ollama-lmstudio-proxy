...
// Upstream `docs/api.md`'s generate-Unload sample carries
...
#[tokio::test]
async fn generate_keep_alive_zero_unload_envelope_matches_upstream_non_streaming() {
    let p = spawn_proxy().await;
    mount_llm_catalog(&p, "llama3.2-3b-instruct").await;
...
#[tokio::test]
async fn generate_keep_alive_zero_unload_envelope_matches_upstream_streaming() {
    let p = spawn_proxy().await;
    mount_llm_catalog(&p, "llama3.2-3b-instruct").await;
